...
import { getAdminTestimonialsFn } from "@/server/admin";
import { useQuery } from "@tanstack/react-query";

function AdminTestimonials() {
  const { data = [], isLoading } = useQuery({
    queryKey: ["admin", "testimonials"],
    queryFn: () => getAdminTestimonialsFn(),
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-display font-medium tracking-tight">Testimonials</h1>
          <p className="text-muted-foreground mt-2">Manage student feedback and reviews.</p>
        </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-muted-foreground">No testimonials found.</td>
                </tr>
              ) : (
                data.map((item) => (
